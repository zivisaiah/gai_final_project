...
class ChatMessage:
    ...
    def __init__(
        self, role: str, content: str, timestamp: datetime = None, metadata: Dict = None
    ):
...
class ChatInterface:
    ...
    def initialize_session_state(self):
        ...
        if "conversation_id" not in st.session_state:
            st.session_state.conversation_id = (
                f"conv_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
    ...
    def display_chat_header(self):
        ...
        if stage == "completed":
            st.success(
                "[SUCCESS] **Interview Scheduled Successfully!** - Conversation Complete"
            )
        elif stage == "ended":
        ...
        with st.sidebar:
            ...
            if st.session_state.scheduling_context["slots_offered"]:
                ...
                st.write(
                    f"**Slots Offered:** {len(st.session_state.scheduling_context['slots_offered'])}"
                )
                if st.session_state.scheduling_context["selected_slot"]:
                    st.write(
                        f"**Selected:** {st.session_state.scheduling_context['selected_slot']}"
                    )
        ...
        if message.role == "user":
            ...
            with st.chat_message("assistant"):
                ...
                if message.metadata:
                    ...
                    # DEBUG: Only log slot debug info for SCHEDULE decisions to reduce noise
                    is_schedule_decision = metadata.get("decision") == "SCHEDULE"
                    
                    if is_schedule_decision:
                        if "suggested_slots" in metadata:
                            print(
                                f"[SEARCH] CHAT DEBUG: Found suggested_slots in metadata: {len(metadata['suggested_slots'])} slots"
                            )
                            if metadata["suggested_slots"]:
                                print(
                                    f"[SEARCH] CHAT DEBUG: First slot: {metadata['suggested_slots'][0]}"
                                )
                            else:
                                print(f"[ERROR] CHAT DEBUG: suggested_slots is empty!")
                        else:
                            print(f"[ERROR] CHAT DEBUG: No suggested_slots key in metadata for SCHEDULE decision")
                            print(
                                f"[X] CHAT DEBUG: Available metadata keys: {list(metadata.keys())}"
                            )

                    if "suggested_slots" in metadata and metadata["suggested_slots"]:
                        st.subheader("[CALENDAR] Available Time Slots")
                        print(
                            f"[OK] CHAT DEBUG: Displaying {len(metadata['suggested_slots'])} slot buttons"
                        )
                        ...
                        for i, slot in enumerate(metadata["suggested_slots"], 1):
                            ...
                            if hasattr(slot, "slot_date") and hasattr(
                                slot, "start_time"
                            ):
                    ...
                    if (
                        "appointment_confirmed" in metadata
                        and metadata["appointment_confirmed"]
                    ):
                        st.success("[PARTY] **Interview Scheduled Successfully!**")
                        if "appointment_details" in metadata:
                            details = metadata["appointment_details"]
                            st.write(
                                f"[CALENDAR] **Date & Time:** {details.get('datetime', 'TBD')}"
                            )
                            st.write(
                                f"[USER] **Interviewer:** {details.get('recruiter', 'TBD')}"
                            )
                            st.write(
                                f"[TIMER] **Duration:** {details.get('duration', 45)} minutes"
                            )
                        st.info(
                            "[OK] **Conversation Complete** - No further action needed."
                        )
    ...
    def handle_user_input(self) -> Optional[str]:
        ...
        if user_input:
            ...
            user_message = ChatMessage(
                role="user", content=user_input, metadata={"input_method": "chat_input"}
            )
    ...
    def add_system_message(self, content: str):
        ...
        system_message = ChatMessage(role="system", content=content)
        st.session_state.messages.append(system_message)

    def handle_slot_selection(self, selected_slot: Dict):
        """Handle when user selects a time slot."""
        # Log the slot selection in the UI component
        if "debug_logger" in st.session_state:
            # Use the specific slot selection logging method
            st.session_state.debug_logger.log_slot_selection(
                selected_slot=selected_slot,
                user_action="button_click",
                metadata={
                    "current_scheduling_context": st.session_state.scheduling_context,
                    "available_slots_in_context": len(
                        st.session_state.scheduling_context.get("slots_offered", [])
                    ),
                },
            )
    ...
    def clear_conversation(self):
        ...
        st.session_state.registration_validation = {"is_valid": False, "errors": []}
    ...
    def export_conversation(self) -> Dict:
        ...
        candidate_info = (
            st.session_state.candidate_info.copy()
        )  # Start with session state

        # If Core Agent exists and has extracted data, use that instead
        if "core_agent" in st.session_state:
            try:
                core_agent_info = st.session_state.core_agent.get_candidate_info(
                    "streamlit_session"
                )
                if core_agent_info:
        ...
        if "debug_logger" in st.session_state:
            ...
            conversation_export["debug_information"] = debug_logger.export_debug_log()
            conversation_export["session_analysis"] = debug_logger.get_session_summary()
    ...
    def display_conversation_stats(self):
        ...
        with st.sidebar:
            ...
            if st.button("📥 Export Conversation with Debug"):
                conversation_data = self.export_conversation()

                # Show debug summary if available
                if "debug_information" in conversation_data:
                    debug_summary = conversation_data["session_analysis"]
                    st.write("**🔍 Debug Summary:**")

                    # Check for slot flow issues
                    slot_analysis = debug_summary.get("slot_flow_analysis", {})
                    if slot_analysis.get("flow_issues"):
                        st.error("⚠️ **Slot Flow Issues Detected:**")
                        for issue in slot_analysis["flow_issues"]:
                            if issue["type"] == "recruiter_mismatch":
                                st.write(
                                    f"- **Recruiter Mismatch:** Selected '{issue['selected_recruiter']}' but booked '{issue['booked_recruiter']}'"
                                )
                    else:
                        st.success("✅ No slot flow issues detected")

                    st.write(
                        f"- **Debug Entries:** {debug_summary['session_info']['total_entries']}"
                    )
                    st.write(
                        f"- **Components:** {', '.join(debug_summary['components_involved'])}"
                    )
                    st.write(f"- **Errors:** {debug_summary['error_count']}")

                # Download button
                st.download_button(
                    label="[FLOPPY] Download Complete Debug Export",
                    data=json.dumps(conversation_data, indent=2, default=str),
                    file_name=f"debug_conversation_{st.session_state.conversation_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                    mime="application/json",
                )

                # Also save to debug logs directory
                if "debug_logger" in st.session_state:
                    debug_logger = st.session_state.debug_logger
                    saved_path = debug_logger.save_debug_export()
                    st.success(f"✅ Debug export also saved to: `{saved_path}`")
    ...
    def display_quick_actions(self):
        ...
        if st.session_state.conversation_stage in ["ended", "completed"]:
            if st.button(
                "🆕 Start New Conversation",
                type="primary",
                key="quick_actions_new_conversation",
            ):
                self.clear_conversation()
                st.rerun()
    ...
    def add_user_quick_message(self, content: str):
        ...
        quick_message = ChatMessage(
            role="user", content=content, metadata={"input_method": "quick_action"}
        )
