...
class ConversationState:
    ...
    def __init__(self, conversation_id: str = None):
        self.conversation_id = (
            conversation_id or f"conv_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def _detect_form_mode(self) -> bool:
        ...
        form_indicators = {
            "position": bool(
                self.candidate_info.get("position")
            ...
            "structured_experience": bool(
                self.candidate_info.get("experience")
                and self.candidate_info.get("experience")
                not in ["unknown", "mentioned"]
            ),  # Structured experience data
            "current_status": bool(
                self.candidate_info.get("current_status")
            ),  # Job status provided
        }
        ...
        is_form_mode = (
            len(active_indicators) >= 2
        )  # At least 2 form indicators suggests form submission
    ...
    def _has_substantial_conversation_data(self) -> bool:
        ...
        meaningful_messages = [
            m for m in user_messages if len(m.get("content", "").strip()) > 10
        ]
    ...
    async def add_message(
        self, role: str, content: str, agent: "CoreAgent", timestamp: datetime = None
    ):
        ...
        if role == "user":
            try:
                ...
                existing_data_count = len(
                    [
                        v
                        for v in self.candidate_info.values()
                        if v not in [None, "", {}, [], "unknown"]
                    ]
                )
                agent.logger.info(
                    f"PRE-EXTRACTION: {existing_data_count} existing data fields: {list(self.candidate_info.keys())}"
                )
                ...
                active_indicators = [
                    key for key, value in form_indicators.items() if value
                ]
                agent.logger.info(
                    f"MODE DETECTION: Form indicators: {form_indicators}, Active: {active_indicators}"
                )

                if is_form_mode:
                    # FORM-BASED MODE: Preserve existing structured data, fill only gaps
                    agent.logger.info(
                        f"[LOCK] FORM-BASED MODE: Preserving {existing_data_count} existing fields, filling gaps only"
                    )
                    agent.logger.info(
                        f"[LOCK] EXISTING DATA TO PRESERVE: {self.candidate_info}"
                    )
                    ...
                    agent.logger.info(
                        f"[CHAT] CONVERSATION-BASED MODE: Full LLM extraction (existing data: {existing_data_count} fields)"
                    )
                    if existing_data_count > 0:
                        agent.logger.warning(
                            f"[!]  CONVERSATION MODE WITH EXISTING DATA: {self.candidate_info}"
                        )
                    await self._handle_conversation_based_extraction(agent)
                else:
                    # MINIMAL DATA MODE: Basic extraction for very limited conversation
                    agent.logger.info(
                        f"🔹 MINIMAL DATA MODE: Basic extraction (existing data: {existing_data_count} fields)"
                    )
                    await self._handle_minimal_extraction(agent)

                # Log what changed
                final_data_count = len(
                    [
                        v
                        for v in self.candidate_info.values()
                        if v not in [None, "", {}, [], "unknown"]
                ...
                agent.logger.info(
                    f"POST-EXTRACTION: {final_data_count} total fields (was {existing_data_count})"
                )
    ...
    async def _handle_form_based_extraction(self, agent: "CoreAgent"):
        ...
        agent.logger.info(
            "[LOCK] FORM-BASED EXTRACTION: Analyzing what needs to be filled"
        )
        ...
        if not self.candidate_info.get("experience") or self.candidate_info.get(
            "experience"
        ) in ["unknown", "mentioned"]:
            missing_fields.append("experience")
        if not self.candidate_info.get("availability_mentioned"):
            missing_fields.append("availability")
        if (
            not self.candidate_info.get("interest_level")
            or self.candidate_info.get("interest_level") == "unknown"
        ):
        ...
        if not missing_fields:
            ...
            agent.logger.info(
                "[OK] NO MISSING FIELDS: Setting HIGH engagement for form-submitted candidate"
            )
        ...
        for field in missing_fields:
            if field in extracted_info and extracted_info[field] not in [
                None,
                "unknown",
                "",
            ]:
                if field == "experience":
                    ...
                    if not current_exp or current_exp in ["unknown", "mentioned"]:
                        agent.logger.info(
                            f"[MEMO] UPDATING EXPERIENCE: '{current_exp}' → '{new_exp}'"
                        )
                        self.candidate_info[field] = new_exp
                        changes_made.append(f"experience: {new_exp}")
                    else:
                        agent.logger.info(
                            f"[LOCK] PRESERVING EXPERIENCE: '{current_exp}' (ignoring extracted: '{new_exp}')"
                        )
                else:
                    ...
                    if not current_val or current_val in [None, "unknown", ""]:
                        agent.logger.info(
                            f"[MEMO] UPDATING {field}: '{current_val}' → '{extracted_info[field]}'"
                        )
                        self.candidate_info[field] = extracted_info[field]
                        changes_made.append(f"{field}: {extracted_info[field]}")
                    else:
                        agent.logger.info(
                            f"[LOCK] PRESERVING {field}: '{current_val}' (ignoring extracted: '{extracted_info[field]}')"
                        )
        ...
        if "conversation_sentiment" in extracted_info:
            ...
            if self._is_form_based_candidate():
                ...
                final_sentiment = {
                    "overall_tone": self._get_better_tone(
                        base_form_sentiment["overall_tone"],
                        llm_sentiment.get("overall_tone", "neutral"),
                    ),
                    "engagement_level": self._get_better_engagement(
                        base_form_sentiment["engagement_level"],
                        llm_sentiment.get("engagement_level", "low"),
                    ),
                    "communication_quality": self._get_better_quality(
                        base_form_sentiment["communication_quality"],
                        llm_sentiment.get("communication_quality", "poor"),
                    ),
                }
                ...
                agent.logger.info(
                    f"[OK] SMART FORM SENTIMENT: Base={base_form_sentiment}, LLM={llm_sentiment}, Final={final_sentiment}"
                )
            else:
        ...
        agent.logger.info(
            f"[OK] FORM-BASED EXTRACTION COMPLETE: Changes made: {changes_made if changes_made else 'None (all data preserved)'}"
        )
        ...
        for key, original_value in original_data.items():
            if original_value not in [None, "", {}, [], "unknown"]:
                ...
                if key == "conversation_sentiment":
                    agent.logger.debug(
                        f"[OK] SENTIMENT EVOLUTION ALLOWED: {key} can change during conversation"
                    )
                    continue
                ...
                if current_value != original_value:
                    agent.logger.error(
                        f"[ALERT] FORM DATA LOST! Field '{key}': '{original_value}' became '{current_value}'"
                    )
                    # Restore the original value
                    self.candidate_info[key] = original_value
                    agent.logger.info(
                        f"[WRENCH] RESTORED: '{key}' back to '{original_value}'"
                    )
                else:
    ...
    async def _handle_conversation_based_extraction(self, agent: "CoreAgent"):
        ...
        agent.logger.info(
            "[CHAT] CONVERSATION-BASED EXTRACTION: Full LLM analysis of conversation context"
        )
        ...
        for key, value in extracted_info.items():
            if value not in [None, "unknown", "", {}, []]:
                ...
                agent.logger.info(
                    f"[MEMO] MERGING FIELD '{key}': existing='{existing_value}' → new='{value}'"
                )
                ...
                if key == "experience":
                    ...
                    if not existing_value or existing_value in ["unknown", "mentioned"]:
                        ...
                        agent.logger.info(
                            f"[OK] UPDATED EXPERIENCE: '{existing_value}' -> '{value}'"
                        )
                    elif (
                        isinstance(value, str)
                        and isinstance(existing_value, str)
                        and "year" in value.lower()
                        and "year" not in existing_value.lower()
                    ):
                        # Prioritize experience with year information
                        self.candidate_info[key] = value
                        agent.logger.info(
                            f"[OK] UPGRADED EXPERIENCE (year info): '{existing_value}' -> '{value}'"
                        )
                    elif (
                        isinstance(value, str)
                        and isinstance(existing_value, str)
                        and len(value) > len(existing_value)
                        and existing_value in ["unknown", "mentioned"]
                    ):
                        self.candidate_info[key] = value
                        agent.logger.info(
                            f"[OK] UPGRADED EXPERIENCE (more detailed): '{existing_value}' -> '{value}'"
                        )
                    else:
                        agent.logger.info(
                            f"[LOCK] PRESERVED EXPERIENCE: '{existing_value}' (ignored: '{value}')"
                        )
                    ...
                    if isinstance(value, dict) and value:
                        self.candidate_info[key] = value
                        agent.logger.info(
                            f"[OK] UPDATED QUALIFICATION ASSESSMENT: {value}"
                        )
                    ...
                    if isinstance(value, dict) and value:
                        ...
                        has_structured_data = (
                            ...
                            and self.candidate_info.get("phone")
                            and self.candidate_info.get("experience")
                            not in [None, "unknown", "mentioned"]
                        )
                        ...
                        if has_structured_data:
                            ...
                            agent.logger.info(
                                f"[OK] FORM-OPTIMIZED CONVERSATION SENTIMENT: {optimized_sentiment}"
                            )
                        else:
                            # Use LLM sentiment for pure conversation users
                            self.candidate_info[key] = value
                            agent.logger.info(
                                f"[OK] UPDATED CONVERSATION SENTIMENT: {value}"
                            )
                    ...
                    if not existing_value or existing_value in [None, "unknown", ""]:
                        self.candidate_info[key] = value
                        agent.logger.info(
                            f"[OK] UPDATED BASIC FIELD '{key}': '{existing_value}' -> '{value}'"
                        )
                    else:
                        agent.logger.info(
                            f"[LOCK] PRESERVED BASIC FIELD '{key}': '{existing_value}' (ignored: '{value}')"
                        )
                    ...
                    if not existing_value:
                        self.candidate_info[key] = value
                        agent.logger.info(
                            f"[OK] UPDATED OTHER FIELD '{key}': '{existing_value}' -> '{value}'"
                        )
            else:
                agent.logger.info(
                    f"⏭️  SKIPPED FIELD '{key}': value is None/empty/unknown ({value})"
                )
    ...
    async def _handle_minimal_extraction(self, agent: "CoreAgent"):
        ...
        if not self.candidate_info.get("name"):
            if any(
                phrase in latest_message.lower()
...
class CoreAgent:
    ...
    def __init__(
        ...
        self.require_contact_info = (
            require_contact_info  # Allow bypassing contact requirement for testing
        )
        ...
        # Debug logger reference (if available)
        ...
        self.logger.info(
            f"Core Agent initialized with {vector_store_type} vector store for Info Advisor"
        )
    ...
    def _log_debug_if_available(
        self, component: str, action: str, data: dict, level: str = "INFO"
    ):
        """Log debug information if debug logger is available."""
        if self.debug_logger:
            self.debug_logger.log_debug(component, action, data, level)

    def _create_safe_llm(
        ...
        try:
            ...
            if "temperature" in str(e).lower() and "unsupported" in str(e).lower():
                self.logger.warning(
                    f"Model {model_name} doesn't support temperature {temperature}, using default temperature (1.0)"
                )
                return ChatOpenAI(
                    api_key=api_key,
                    model=model_name,
...
Analyze this context and respond with the JSON decision format only.""",
    ...
    async def _assess_candidate_qualifications(
        ...
        if (
            "qualification_assessment" in candidate_info
            and candidate_info["qualification_assessment"]
        ):
            existing_assessment = candidate_info["qualification_assessment"]

            # Validate assessment completeness
            if (
                existing_assessment.get("qualification_status", "unknown") != "unknown"
                and existing_assessment.get("assessment_confidence", 0) > 0.3
            ):
                self.logger.info(
                    f"Using existing qualification assessment: {existing_assessment}"
                )
                return existing_assessment

        # If no valid assessment exists, trigger protected LLM extraction
        self.logger.info(
            "No valid qualification assessment found, triggering protected LLM extraction"
        )
        ...
        try:
            ...
            enhanced_info = await self.extract_qualification_assessment_llm(
                conversation
            )

            # SELECTIVE UPDATE: Only add qualification assessment, preserve everything else
            if (
                "qualification_assessment" in enhanced_info
                and enhanced_info["qualification_assessment"]
            ):
                conversation.candidate_info["qualification_assessment"] = enhanced_info[
                    "qualification_assessment"
                ]
                assessment = enhanced_info["qualification_assessment"]

                self.logger.info(
                    f"[OK] SAFE ASSESSMENT UPDATE: Added qualification only"
                )
            ...
            self.logger.error(
                f"[WRENCH] RESTORED original candidate data after assessment error: {e}"
            )
    ...
    def _create_safe_fallback_assessment(
        self, error_msg: str = "Assessment unavailable"
    ) -> Dict[str, Any]:
    ...
    async def process_message_async(
        ...
        try:
            ...
            entry_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
            ...
            if (
                len(messages) > self.max_history_length * 2
            ...
            qualification_assessment = await self._assess_candidate_qualifications(
                conversation
            )
    ...
    async def _make_decision(
        ...
        # CRITICAL DEBUG: Log _make_decision entry
        self.logger.info(f"[_MAKE_DECISION] ENTRY: user_message='{user_message[:100]}...', conversation_id={conversation.conversation_id}")
        self.logger.info(f"[_MAKE_DECISION] Candidate info keys: {list(conversation.candidate_info.keys())}")
        if "available_slots" in conversation.candidate_info:
            self.logger.info(f"[_MAKE_DECISION] Available slots count: {len(conversation.candidate_info['available_slots'])}")
        else:
            self.logger.info(f"[_MAKE_DECISION] NO available_slots in candidate_info")
        
        try:
            # Check for significant qualification mismatch early in conversation
            qualification_assessment = conversation.candidate_info.get(
                "qualification_assessment", {}
            )
            ...
            has_concrete_experience = (
                candidate_experience
                and candidate_experience not in [None, "unknown", "mentioned", ""]
            )

            # If candidate is underqualified AND we have concrete experience data, be proactive
            if (
                qualification_status == "underqualified"
                and has_concrete_experience
                and experience_gap  # Must have actual experience info, not null/unknown
                >= 1
                and len(  # 1+ year gap is significant for junior-mid level positions
                    conversation.messages
                )
                <= 4
                and not any(  # Early in conversation
                    "qualification" in msg.get("content", "").lower()
                    or "experience" in msg.get("content", "").lower()
                    or "requirement" in msg.get("content", "").lower()
                    for msg in conversation.messages[-3:]
                    if msg.get("role") == "assistant"
                )
            ):
                self.logger.info(
                    f"Proactively addressing qualification mismatch: {experience_gap} year gap"
                )

                # Calculate actual years from concrete experience data (not assumptions)
                experience_details = conversation.candidate_info.get(
                    "experience_details", {}
                )
                ...
                if years_numeric is not None:
                    ...
                    actual_years = (
                        int(years_match.group(1)) if years_match else "unclear"
                    )
...
What specific Python projects or technologies have you worked with in your {actual_years} years of experience?"""
            ...
            chain_input = {
                ...
                "candidate_info": conversation.candidate_info,
                "conversation_context": self.prompts.format_conversation_context(
                    conversation.messages
                ),
            }
            ...
            decision, reasoning, agent_response = self._parse_agent_response(
                response_text
            )
            ...
            if decision == AgentDecision.INFO:
                self.logger.info(
                    "Decision is INFO. Consulting Info Advisor for job-related answer."
                )
                ...
                try:
                    ...
                    info_response: InfoResponse = (
                        await self.info_advisor.answer_question(
            ...
            # --- Proactive Scheduling Logic ---
                ...
                self.logger.info(
                    "Decision is SCHEDULE. Consulting SchedulingAdvisor for available slots."
                )
                
                # CRITICAL DEBUG: Log SCHEDULE decision path entry
                self.logger.info(f"[SCHEDULE_PATH] ENTRY: Processing SCHEDULE decision for message: '{user_message}'")
                self.logger.info(f"[SCHEDULE_PATH] Conversation has {len(conversation.candidate_info)} candidate info fields")
                self.logger.info(f"[SCHEDULE_PATH] Current available_slots in conversation: {len(conversation.candidate_info.get('available_slots', []))} slots")
                ...
                # CRITICAL DEBUG: Log which scheduling path is taken
                self.logger.info(f"[SCHEDULE_PATH] SchedulingAdvisor decision: {schedule_decision}")
                self.logger.info(f"[SCHEDULE_PATH] SchedulingAdvisor reasoning: {schedule_reasoning}")
                self.logger.info(f"[SCHEDULE_PATH] Available slots from advisor: {len(available_slots)}")
                ...
                if schedule_decision == SchedulingDecision.CONFIRM_SLOT:
                    ...
                    self.logger.info(
                        "SchedulingAdvisor detected slot confirmation. Triggering booking process."
                    )
                    self.logger.info(f"[CONFIRM_SLOT] Processing slot confirmation for user message: '{user_message}'")

                    # Try to match the user's confirmation to a specific slot
                    # CRITICAL DEBUG: Log slot confirmation inputs
                    self.logger.info(f"[SLOT_CONFIRMATION] Input available_slots count: {len(available_slots)}")
                    for i, slot in enumerate(available_slots):
                        slot_recruiter = slot.get("recruiter") if isinstance(slot, dict) else getattr(slot, "recruiter", {}).get("name", "Unknown") if hasattr(slot, "recruiter") else "Unknown"
                        self.logger.info(f"[SLOT_CONFIRMATION] Input slot {i}: {slot_recruiter}")
                    
                    booking_result = await self._handle_slot_confirmation(
                        conversation, user_message, available_slots
                    )
                    
                    # CRITICAL DEBUG: Log booking result
                    self.logger.info(f"[SLOT_CONFIRMATION] Booking result: {booking_result.get('success')}")
                    if booking_result.get("success"):
                        booked_recruiter = booking_result.get("appointment_details", {}).get("recruiter", "Unknown")
                    ...
                    schedule_decision == SchedulingDecision.SCHEDULE and available_slots
                ):
                    # CRITICAL DEBUG: Log slot generation path
                    self.logger.info(f"[SLOT_GENERATION] Processing new slot generation")
                    self.logger.info(f"[SLOT_GENERATION] Got {len(available_slots)} slots from SchedulingAdvisor")
                    for i, slot in enumerate(available_slots):
                        slot_recruiter = slot.get("recruiter") if isinstance(slot, dict) else getattr(slot, "recruiter", {}).get("name", "Unknown") if hasattr(slot, "recruiter") else "Unknown"
                        self.logger.info(f"[SLOT_GENERATION] Slot {i}: {slot_recruiter}")
                    
                    # TIMING DEBUG: Log slot storage timing
                    storage_start = time.time()
                    storage_timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                    ...
                    # CRITICAL DEBUG: Verify slot storage immediately
                    stored_check = conversation.candidate_info.get("available_slots", [])
                    self.logger.info(f"[SLOT STORAGE] IMMEDIATE CHECK: Stored {len(stored_check)} slots in conversation")
                    
                    # Also verify it's stored in the main conversations dict
                    conv_from_dict = self.conversations.get(conversation.conversation_id)
                    if conv_from_dict:
                        dict_check = conv_from_dict.candidate_info.get("available_slots", [])
                        self.logger.info(f"[SLOT STORAGE] DICT CHECK: {len(dict_check)} slots in conversations dict")
                        if len(dict_check) != len(available_slots):
                            self.logger.error(f"[SLOT STORAGE] MISMATCH: Direct={len(stored_check)}, Dict={len(dict_check)}")
                    else:
                        self.logger.error(f"[SLOT STORAGE] Conversation not found in conversations dict!")
                    ...
                    self._log_debug_if_available(
                        "core_agent_timing",
                        "slot_storage_complete",
                        {
                            "timestamp": end_timestamp,
                            "duration_ms": storage_duration,
                            "slots_stored": len(available_slots),
                            "conversation_id": conversation.conversation_id,
                        },
                    )
                    ...
                    if (
                        "no available slots" in schedule_reasoning.lower()
                        or "no slots" in schedule_reasoning.lower()
                    ):
            ...
            if decision == AgentDecision.CONTINUE and conversation.candidate_info.get(
                ...
                contact_request_response = await self._generate_contact_info_request(
            ...
            method_duration = (method_end - method_start) * 1000  # ms
            ...
            self._log_debug_if_available(
                ...
                "process_message_exit",
                {
                    ...
                    "decision": decision.value,
                    "conversation_id": conversation.conversation_id or "default",
                },
            )
    ...
    def _parse_agent_response(
        ...
        try:
            ...
            self.logger.error(
                f"Critical error parsing LLM JSON response: {e}. Raw response: {response_text}"
            )
            # Re-raise to be caught by the main error handler. We no longer use a fallback.
            raise ValueError(
                f"Could not parse decision from LLM response: {response_text}"
            ) from e

    def _validate_decision(
        self, decision: AgentDecision, conversation: ConversationState
    ) -> AgentDecision:
        ...
        has_experience = bool(
            ...
            and candidate_info.get("experience") not in ["unknown", ""]
        )
        ...
        self.logger.info(
            f"Validation check - Name: {has_name}, Experience: {has_experience}, "
            f"Availability: {has_availability}, Interest: {has_interest}"
        )
        self.logger.info(
            f"Contact validation - Email: {has_email}, Phone: {has_phone}, HasContact: {has_contact_info}"
        )
        ...
        if (
            ...
            self.logger.info(
                "Overriding CONTINUE to SCHEDULE based on sufficient candidate information"
            )
            return AgentDecision.SCHEDULE

        # NEW: Block SCHEDULE decisions if missing contact information (unless disabled for testing)
        if (
            decision == AgentDecision.SCHEDULE
            and self.require_contact_info
            and not has_contact_info
        ):
            self.logger.info(
                "Blocking SCHEDULE decision - missing contact information (email or phone)"
            )
    ...
    async def _handle_no_slots_available(
        ...
        try:
            ...
            flexibility_attempts = sum(
                ...
                if msg.get("role") == "assistant"
                and (
                    "flexibility" in msg.get("content", "").lower()
                    or "alternative" in msg.get("content", "").lower()
                )
            )
            ...
            if flexibility_attempts == 0:
                ...
                return await self._trigger_scheduling_exit(
                    conversation, schedule_reasoning
                )
    ...
    async def _ask_for_flexibility(
        self, conversation: ConversationState, schedule_reasoning: str
    ) -> str:
        ...
        for msg in conversation.messages[-3:]:
            if msg.get("role") == "user":
                ...
                if "between" in content and ("pm" in content or "am" in content):
                    user_preference = (
                        f"the time you mentioned ({msg.get('content', '')})"
                    )
                    break
    ...
    async def _offer_alternatives(
        ...
        try:
            ...
            available_slots = self.scheduling_advisor.sql_manager.get_available_slots(
                start_date=today, end_date=end_date, available_only=True
            )[
                :3
...
If none of these times work, we may need to explore other options or schedule for a later date."""
    ...
    async def _handle_slot_confirmation(
        ...
        # Log slot confirmation start
        self._log_debug_if_available(
            "core_agent_slot_confirmation",
            "confirmation_start",
            {
                ...
                "available_slots_details": [
                    {
                        ...
                        else getattr(slot, "id", None),
                        "recruiter": slot.get("recruiter")
                        if isinstance(slot, dict)
                        else getattr(slot, "recruiter", {}).get("name", "Unknown")
                        if hasattr(slot, "recruiter")
                        else "Unknown",
                        "datetime": slot.get("datetime")
                        if isinstance(slot, dict)
                        else f"{getattr(slot, 'slot_date', '')}T{getattr(slot, 'start_time', '')}",
                    }
                    for slot in available_slots
                ],
            },
...
} for i, slot in enumerate(available_slots)], indent=2)}

CRITICAL: When matching slots, you MUST consider BOTH the time/date AND the recruiter name if mentioned.
If the user mentions a specific recruiter name (like "Sarah Johnson" or "Mike Chen"), you MUST match that exact recruiter.

Extract the following information:
1. Which slot is the user selecting? (match by time, day, AND recruiter name if specified)
2. What time did they mention? (if any)
3. What day/date did they mention? (if any)
4. What recruiter did they mention? (if any)
5. Are they asking a question instead of confirming?
...
{{
  ...
  "mentioned_recruiter": "exact recruiter name mentioned" or null,
  "confidence": 0.0 to 1.0,
  "reasoning": "brief explanation of your selection including time AND recruiter matching"
}}
"""

            # Get LLM analysis
            response = await self.candidate_info_chain.ainvoke(
                {"extraction_prompt": slot_parsing_prompt}
            )
            response_text = response.content.strip()

            # Log LLM parsing start
            self._log_debug_if_available(
                "core_agent_slot_confirmation",
                "llm_parsing_start",
                {
                    "prompt_sent": slot_parsing_prompt[:200] + "...",
            ...
            try:
                ...
                # Log LLM parsing result
                self._log_debug_if_available(
                    "core_agent_slot_confirmation",
                    "llm_parsing_result",
                    {
                        "slot_selection_parsed": slot_selection,
                        "is_confirmation": slot_selection.get("is_confirmation", False),
                        "selected_slot_index": slot_selection.get(
                            "selected_slot_index", -1
                        ),
                        "confidence": slot_selection.get("confidence", 0.0),
                    },
                )
                ...
                if selected_index < 0 or selected_index >= len(available_slots):
                    mentioned_time = slot_selection.get(
                        "mentioned_time", "your preferred time"
                    )
                    return {
                        "success": False,
                ...
                # CRITICAL DEBUG: Log LLM parsing results
                self.logger.info(f"[LLM_SLOT_PARSING] Selected index: {selected_index}")
                self.logger.info(f"[LLM_SLOT_PARSING] Mentioned recruiter: {slot_selection.get('mentioned_recruiter')}")
                self.logger.info(f"[LLM_SLOT_PARSING] Confidence: {slot_selection.get('confidence')}")
                self.logger.info(f"[LLM_SLOT_PARSING] Reasoning: {slot_selection.get('reasoning')}")

                # CRITICAL VALIDATION: Ensure recruiter matches if mentioned in user message
                mentioned_recruiter = slot_selection.get("mentioned_recruiter")
                if mentioned_recruiter:
                    slot_recruiter = best_match.get("recruiter") if isinstance(best_match, dict) else getattr(best_match, "recruiter", {}).get("name", "Unknown") if hasattr(best_match, "recruiter") else "Unknown"
                    
                    # Check if recruiter names match (case-insensitive)
                    if mentioned_recruiter.lower() not in slot_recruiter.lower():
                        self.logger.warning(f"RECRUITER MISMATCH: User mentioned '{mentioned_recruiter}' but LLM selected slot with '{slot_recruiter}' (index {selected_index})")
                        
                        # Try to find the correct slot with matching recruiter
                        correct_slot_index = -1
                        for i, slot in enumerate(available_slots):
                            slot_rec = slot.get("recruiter") if isinstance(slot, dict) else getattr(slot, "recruiter", {}).get("name", "Unknown") if hasattr(slot, "recruiter") else "Unknown"
                            if mentioned_recruiter.lower() in slot_rec.lower():
                                correct_slot_index = i
                                best_match = available_slots[i]
                                selected_index = i
                                self.logger.info(f"RECRUITER FIX: Corrected to slot index {i} with recruiter '{slot_rec}'")
                                break
                        
                        if correct_slot_index == -1:
                            return {
                                "success": False,
                                "error": f"I couldn't find a slot with {mentioned_recruiter}. Please select from the available options shown above.",
                            }

                # CRITICAL FIX: Ensure best_match is a proper dictionary with all required fields
                if hasattr(best_match, "id"):
                    # Convert AvailableSlotResponse object to dictionary
                    best_match_dict = {
                        "id": best_match.id,
                        "datetime": datetime.combine(
                            best_match.slot_date, best_match.start_time
                        ).isoformat(),
                        "recruiter": best_match.recruiter.name
                        if best_match.recruiter
                        else "Unknown",
                        "recruiter_id": best_match.recruiter_id,
                        "is_available": best_match.is_available,
                    }
                    best_match = best_match_dict

                # Log slot selection details
                selected_slot_details = {
                    "selected_index": selected_index,
                    "selected_slot_data": best_match,
                    "slot_id": best_match.get("id")
                    if isinstance(best_match, dict)
                    else getattr(best_match, "id", None),
                    "recruiter": best_match.get("recruiter")
                    if isinstance(best_match, dict)
                    else getattr(best_match, "recruiter", {}).get("name", "Unknown")
                    if hasattr(best_match, "recruiter")
                    else "Unknown",
                    "datetime": best_match.get("datetime")
                    if isinstance(best_match, dict)
                    else f"{getattr(best_match, 'slot_date', '')}T{getattr(best_match, 'start_time', '')}",
                    "recruiter_id": best_match.get("recruiter_id")
                    if isinstance(best_match, dict)
                    else getattr(best_match, "recruiter_id", None),
                }
            ...
            slot_datetime = datetime.fromisoformat(
                best_match["datetime"].replace("Z", "+00:00")
            )
            recruiter_id = best_match.get("recruiter_id", 1)
            slot_id = best_match.get("id")

            # Log booking attempt
            booking_attempt_data = {
                "candidate_info_keys": list(candidate_info.keys()),
                "slot_datetime": slot_datetime.isoformat(),
                "recruiter_id": recruiter_id,
                "slot_id": slot_id,
                "expected_recruiter": best_match.get("recruiter")
                if isinstance(best_match, dict)
                else getattr(best_match, "recruiter", {}).get("name", "Unknown")
                if hasattr(best_match, "recruiter")
                else "Unknown",
            ...
            self._log_debug_if_available(
                "core_agent_slot_confirmation", "booking_attempt", booking_attempt_data
            )
            ...
            self._log_debug_if_available(
                ...
                "booking_result",
                {
                    ...
                    "returned_recruiter": booking_result.get("recruiter", {}).get(
                        "name", "Unknown"
                    )
                    if booking_result.get("recruiter")
                    else "Unknown",
                    "booking_result_full": booking_result,
                },
            )

            if booking_result["success"]:
                self.logger.info(
                    f"Successfully booked appointment {booking_result.get('appointment_id')}"
                )

                # Update conversation state to mark as completed
                conversation.candidate_info["appointment_booked"] = True
                final_appointment_details = {
                    "datetime": slot_datetime.strftime("%A, %B %d, %Y at %I:%M %p"),
                    "recruiter": booking_result.get("recruiter", {}).get(
                        "name", "Our recruiter"
                    ),
                    "appointment_id": booking_result.get("appointment_id"),
                }
                ...
                self._log_debug_if_available(
                    ...
                    "final_appointment_details",
                    {
                        ...
                        "final_recruiter_from_booking": booking_result.get(
                            "recruiter", {}
                        ).get("name", "Our recruiter"),
                        "final_appointment_details": final_appointment_details,
                        "potential_mismatch": (
                            best_match.get("recruiter")
                            if isinstance(best_match, dict)
                            else getattr(best_match, "recruiter", {}).get(
                                "name", "Unknown"
                            )
                            if hasattr(best_match, "recruiter")
                            else "Unknown"
                        )
                        != booking_result.get("recruiter", {}).get(
                            "name", "Our recruiter"
                        ),
                    },
                )
                ...
                return {
                    ...
                    "appointment_details": final_appointment_details,
                }
            else:
                return {
                    "success": False,
            ...
            return {"success": False, "error": f"Booking system error: {str(e)}"}
...
This will help me determine the best next steps for us."""
    ...
    async def _trigger_scheduling_exit(
        self, conversation: ConversationState, schedule_reasoning: str
    ) -> str:
        """Trigger exit conversation when scheduling repeatedly fails."""
        self.logger.info(
            "Triggering exit conversation due to repeated scheduling failures"
        )
...
Thank you for your time, and I wish you the best in your job search!"""
    ...
    async def _generate_contact_info_request(
        self, conversation: ConversationState, user_message: str
    ) -> str:
...
Once I have your contact details, I'll be able to show you available time slots and confirm your interview!"""
    ...
    async def extract_missing_info_llm(
        self, conversation: ConversationState, missing_fields: List[str]
    ) -> Dict:
        """
        ...
        try:
            ...
            fields_description = {
                ...
                "availability": "ANY mention of when they can meet, time preferences, or scheduling availability (like 'after 10am', 'any day after X', 'weekdays', 'anytime', 'available', 'free', 'morning', 'afternoon', etc.)",
...
{{
  ...
  "availability": true if ANYTHING about when they can meet is mentioned (like 'after 10am', 'any day after X', 'weekdays', 'anytime', 'available', 'free', 'morning', 'afternoon'), false only if no time/availability info given,
...
Extract only what is clearly mentioned in the conversation and NOT already in the form data. JSON_RESPONSE_ONLY:"""
            ...
            # Get LLM response
            response = await self.candidate_info_chain.ainvoke(
                {"extraction_prompt": targeted_prompt}
            )
            response_text = response.content.strip()
            ...
            if "availability" in extracted_data:
                extracted_data["availability_mentioned"] = extracted_data.pop(
                    "availability"
                )

            self.logger.info(
                f"Targeted extraction for missing fields {missing_fields}: {extracted_data}"
            )
    ...
    async def extract_simple_info_llm(
        self, conversation: ConversationState, fields: List[str]
    ) -> Dict:
        """
        ...
        try:
            latest_message = (
                conversation.messages[-1].get("content", "")
...
{{
  ...
  "basic_info": "any other info or null"
            ...
            response = await self.candidate_info_chain.ainvoke(
                {"extraction_prompt": simple_prompt}
            )
            response_text = response.content.strip()
    ...
    async def extract_qualification_assessment_llm(
        self, conversation: ConversationState
    ) -> Dict:
        """
...
JSON_RESPONSE_ONLY:"""
            ...
            # Get LLM response
            response = await self.candidate_info_chain.ainvoke(
                {"extraction_prompt": assessment_prompt}
            )
            response_text = response.content.strip()
            ...
            if "qualification_assessment" in assessment_data:
                ...
                assessment["should_continue"] = not assessment.get(
                    "meets_requirements", True
                )

                self.logger.info(
                    f"[OK] QUALIFICATION ASSESSMENT EXTRACTED: {assessment}"
                )
...
Use the enhanced extraction format but maintain all existing data integrity. Respond with valid JSON only."""
                ...
                extraction_prompt = self.prompts.get_candidate_info_extraction_prompt(
                    conversation.messages
                )
            ...
            if isinstance(raw_experience, str):
                experience_data = {
                    "years": raw_experience
                    if raw_experience not in ["unknown", ""]
                    else None,
                    "level": None,
                    "years_numeric": self._extract_years_from_string(raw_experience),
                    "technologies": ["python"]
                    if raw_experience and "python" in raw_experience.lower()
                    else [],
            ...
            candidate_info = {
                ...
                "availability_mentioned": extracted_data.get(
                ...
                "qualification_assessment": {
                    "meets_requirements": qualification_data.get(
                        "meets_requirements", False
                    ),
                    "experience_gap": qualification_data.get("experience_gap_years", 0),
                    "qualification_status": qualification_data.get(
                        "qualification_status", "unknown"
                    ),
                    "assessment_confidence": qualification_data.get(
                        "assessment_confidence", 0.0
                    ),
                    "key_concerns": qualification_data.get("key_concerns", []),
                    "strengths": qualification_data.get("strengths", []),
                    "should_continue": not qualification_data.get(
                        "meets_requirements", True
                    ),  # Continue if underqualified
                    "assessment_reason": self._generate_assessment_reason(
                        qualification_data, experience_data
                    ),
                },
                # Conversation context
                "conversation_sentiment": extracted_data.get(
                    "conversation_sentiment", {}
                ),
            ...
            self.logger.info(
                f"Qualification assessment: {candidate_info['qualification_assessment']}"
            )
            ...
            self.logger.error(
                f"Raw response that caused error: {response_text if 'response_text' in locals() else 'N/A'}"
            )
            ...
            return {
                ...
                "qualification_assessment": {
                    ...
                    "assessment_confidence": 0.0,
                    "key_concerns": [
                        "Unable to assess qualifications due to extraction error"
                    ],
                    "strengths": [],
                    "should_continue": True,
    ...
    def _generate_assessment_reason(
        self, qualification_data: Dict, experience_data: Dict
    ) -> str:
    ...
    def get_candidate_info(self, conversation_id: str) -> Dict:
        ...
        if conversation:
            candidate_info = conversation.candidate_info
            available_slots_count = len(candidate_info.get("available_slots", []))
            self.logger.info(f"[GET_CANDIDATE_INFO] Returning {len(candidate_info)} fields for {conversation_id}")
            self.logger.info(f"[GET_CANDIDATE_INFO] Available slots: {available_slots_count}")
            if available_slots_count > 0:
                self.logger.info(f"[GET_CANDIDATE_INFO] First slot: {candidate_info['available_slots'][0]}")
            return candidate_info
        else:
            self.logger.error(f"[GET_CANDIDATE_INFO] No conversation found for {conversation_id}")
            return {}
